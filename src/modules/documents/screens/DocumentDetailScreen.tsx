...
import { useDocumentDetail } from '@/modules/documents/hooks/useDocumentDetail';
...
export default function DocumentDetailScreen() {
    ...
    useEffect(() => {
        if (doc?.linkedDocs) {
            const valid = doc.linkedDocs.filter(ld => ld && ld.id && ld.name);
            setLinkedDocs(valid);
        }
    }, [doc?.linkedDocs]);
                ...
                <View style={styles.section}>
                    ...
                    <View style={styles.fieldsGrid}>
                        ...
                        <InfoField
                            ...
                            value={doc.paymentMethod || 'No especificado'}
                    ...
                    {doc.billingUrl ? (
                        <View style={styles.billingRow}>
                            <View style={[styles.infoFieldIcon, { backgroundColor: '#F0F9FF' }]}>
                                <Ionicons name="globe-outline" size={13} color="#0EA5E9" />
                            </View>
                            <Text style={styles.billingLabel}>Página de facturación</Text>
                            <TouchableOpacity
                                onPress={async () => {
                                    await Clipboard.setStringAsync(doc.billingUrl!);
                                    Alert.alert(
                                        'Enlace copiado',
                                        'La URL de facturación se ha copiado al portapapeles.',
                                    );
                                }}
                                activeOpacity={0.7}
                                style={styles.billingLink}
                            >
                                <Text style={styles.billingLinkText} numberOfLines={1}>
                                    {doc.billingUrl}
                                </Text>
                            ...
                            <View style={styles.productsList}>
                                {doc.items.map((item, idx) => (
                                    <View
                                        ...
                                        style={[
                                            styles.productRow,
                                            idx < doc.items.length - 1 && styles.productRowBorder,
                                        ]}
                                    >
...
const styles = StyleSheet.create({
    ...
    billingLinkText: {
        ...
        flex: 1, textDecorationLine: 'underline',
    },
    ...
    productRowBorder: { borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },
    productIcon: {
        width: 32, height: 32, borderRadius: 8,
        backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center',
    },
    productInfo: { flex: 1, minWidth: 0, gap: 2 },
    productName: { fontSize: 13, fontWeight: '600', color: '#1E293B' },
    productMeta: { fontSize: 11, color: '#94A3B8' },
    productPrice: { fontSize: 13, fontWeight: '700', color: '#0F172A' },
